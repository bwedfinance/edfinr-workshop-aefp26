...
# The story in the data {background-color="#007786"}
...
1. Pull multi-year pandemic-era data with **range syntax**
2. Clean and select key variables
3. Calculate per-pupil expenditures with `across()`
4. Explore COVID spending vs. student poverty
5. Filter to a single state for deeper analysis

# Getting the data {background-color="#007786"}
...
# COVID spending and poverty {background-color="#007786"}
...
```{r}
#| eval: false
#| code-line-numbers: "|1-6|7-14|15-16"
  geom_smooth(
    aes(
      x = stpov_pct,
      y = exp_covid_total_pp
...
- **Color gradient**: Wide variation in instructional share — some districts used ESSER mostly for instruction, others for facilities or staffing
...
The scatter plot tells a **richer story** than a bar chart — we see the relationship between poverty and spending, not just totals.

# Zooming into one state {background-color="#007786"}
...
## Your turn! {background-color="#007786"}

### Time for independent state analysis

Open the exercise file and pick a state to explore.
